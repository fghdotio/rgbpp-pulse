...
const operationIcons: Record<ActivityOperation, typeof ArrowLeftRight> = {
...
export function TransactionList({
  ...
  const [allTxs, setAllTxs] = useState<ActivityEntry[]>([]);
  ...
  const fetchInitial = useCallback(async (force?: boolean) => {
    ...
    setError(null);
    try {
      const result = await indexer.getAddressActivity(btcAddress);
      // Only apply results if this is still the latest fetch
      if (fetchIdRef.current !== id) return;
      setAllTxs(result.entries);
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const result = await indexer.getAddressActivity(btcAddress, { cursor });
      setAllTxs((prev) => [...prev, ...result.entries]);
      setCursor(result.nextCursor ?? undefined);
      setHasMore(!!result.nextCursor);
    } catch (err) {
  ...
  // ── Collect BTC txids from indexer results for dedup ──────
  const remoteBtcTxIds = useMemo(
    () =>
      new Set(
        allTxs.map((tx) => tx.btc?.txid).filter((txid): txid is string => !!txid)
      ),
    [allTxs]
  );
  ...
  // Filter indexed transactions. Everything here has a confirmed CKB transition
  // — the indexer stays behind the tip by its reorg lag — so the pending filter
  // leaves only local pipelines.
  const filteredApiTxs = useMemo(() => {
    if (statusFilter === "pending") return [];
    return allTxs.filter((tx) => {
      if (operationFilter !== "all" && activityOperation(tx) !== operationFilter) {
        return false;
      }
      return true;
    });
      ...
      {filteredApiTxs.length > 0 && (
        <div className="space-y-3">
          {filteredApiTxs.map((tx) => {
            const txType = activityOperation(tx);
            const Icon = operationIcons[txType];
            const label = operationLabels[txType];
            const assetType = activityAssetType(tx);
            const isConfirmed = activityConfirmed(tx);
            const timestamp = activityTimestamp(tx);
            const fee = activityFee(tx);
            const btcTxid = tx.btc?.txid;
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <a
                          href={`https://testnet.explorer.nervos.org/transaction/${tx.ckbTxHash}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1 text-xs text-muted-foreground hover:text-primary transition-colors font-mono"
                        >
                          CKB {truncateAddress(tx.ckbTxHash, 6, 4)}
                          <ExternalLink className="size-3" />
                        </a>
                      </div>
                    </div>
                    ...
                    <div className="text-right shrink-0">
                      ...
                      {tx.btc?.blockHeight != null && (
                        <p className="text-[11px] text-muted-foreground mt-0.5 font-mono">
                          #{tx.btc.blockHeight.toLocaleString()}
                        </p>
                      )}
                    </div>
                  </div>
